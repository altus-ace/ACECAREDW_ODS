CREATE TABLE [adi].[MbrUhcMbrHighRisk] (
    [mbrUhcHighRiskKey]    INT           IDENTITY (1, 1) NOT NULL,
    ...
    [InQuarantine]         TINYINT       CONSTRAINT [DF_astMbrUHCMbrHighRisk_InQuarantine] DEFAULT ((0)) NOT NULL,
    [LoadDate]             DATE          NOT NULL,
    [DataDate]             DATE          NOT NULL,
    [CreateDate]           DATETIME2 (7) CONSTRAINT [Df_astMbrUHCMbrHighRisk_CreateDate] DEFAULT (getdate()) NOT NULL,
    [CreateBy]             VARCHAR (50)  CONSTRAINT [Df_astMbrUHCMbrHighRisk_CreateBy] DEFAULT (suser_sname()) NOT NULL,
    [LastUpdatedDate]      DATETIME2 (7) CONSTRAINT [DF_astMbrUHCMbrHighRisk_LastUpdatedDate] DEFAULT (getdate()) NOT NULL,
    [LastUpdatedBy]        VARCHAR (50)  CONSTRAINT [DF_astMbrUHCMbrHighRisk_LastUpdatedBy] DEFAULT (suser_sname()) NOT NULL,
    CONSTRAINT [PK_adiMbrUhcMbrHighRisk_mbrUhcHighRiskKey] PRIMARY KEY CLUSTERED ([mbrUhcHighRiskKey] ASC),
    CONSTRAINT [Ck_astMbrUHCMbrHighRisk_InQuarantine] CHECK ([InQuarantine]=(1) OR [InQuarantine]=(0))
);


GO

CREATE TRIGGER adi.[UpdateMbrUhcMbrHighRisk_LastUpDated]
ON adi.MbrUhcMbrHighRisk
AFTER UPDATE 
AS
   UPDATE adi.MbrUhcMbrHighRisk
   SET [LastUpdatedDate] = SYSDATETIME()
    , [LastUpdatedBy] = SYSTEM_USER
   FROM Inserted i
   WHERE adi.MbrUhcMbrHighRisk.mbrUhcHighRiskKey = i.mbrUhcHighRiskKey
