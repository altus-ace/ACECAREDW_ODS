CREATE TABLE [adi].[mbrUhcMbrByPcp] (
    [mbrMbrByPcpKey]       INT           IDENTITY (1, 1) NOT NULL,
    ...
    [CreateDate]           DATETIME2 (7) CONSTRAINT [Df_adiMbrUHCMbrByPcp_CreateDate] DEFAULT (getdate()) NOT NULL,
    [CreateBy]             VARCHAR (50)  CONSTRAINT [Df_adiMbrUHCMbrByPcp_CreateBy] DEFAULT (suser_sname()) NOT NULL,
    [LastUpdatedDate]      DATETIME2 (7) CONSTRAINT [DF_adiMbrUHCMbrByPcp_LastUpdatedDate] DEFAULT (getdate()) NOT NULL,
    [LastUpdatedBy]        VARCHAR (50)  CONSTRAINT [DF_adiMbrUHCMbrByPcp_LastUpdatedBy] DEFAULT (suser_sname()) NOT NULL,
    [COB_FLAG]             VARCHAR (20)  NULL,
    [MbrLoadStatus]        TINYINT       CONSTRAINT [DF_adiMbrUhcMbrByPcp_MbrLoadStatus] DEFAULT ((0)) NOT NULL,
    CONSTRAINT [PK_MbrUhcMbrByPcp] PRIMARY KEY CLUSTERED ([mbrMbrByPcpKey] ASC),
    CONSTRAINT [CK_adiMbrUHCMbrByPcp_LoadType] CHECK ([LoadType]='S' OR [LoadType]='P'),
    CONSTRAINT [CK_mbrUhcMbrByProvider_LoadType] CHECK ([LoadType]='S' OR [LoadType]='P')
);


GO
CREATE TRIGGER adi.[UpdateMbrUhcMbrByPcp_LastUpDated]
ON adi.MbrUhcMbrByPcp
AFTER UPDATE 
AS
   UPDATE adi.MbrUhcMbrByPcp
   SET [LastUpdatedDate] = SYSDATETIME()
    , [LastUpdatedBy] = SYSTEM_USER
   FROM Inserted i
   WHERE adi.MbrUhcMbrByPcp.[mbrMbrByPcpKey]= i.[mbrMbrByPcpKey]
