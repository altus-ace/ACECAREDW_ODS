CREATE FUNCTION [adi].[tvf_PdwSrc_Wlc_MemberByPcp_OLD]( 
    @LoadDate DATE
    )
    RETURNS TABLE
    AS 
    RETURN
       --declare @loadDate date = '2021-03-30' 
    ...
    FROM adi.MbrWlcMbrByPcp adiMbrs -- list of members
	   JOIN lst.List_Client Client 
        ...
        JOIN (/* What is the sub select for */
			 ...
			 , m.EffDate, ISNULL(m.TermDate, '12/31/2099') TERMDATE
			 , m.ADDRESS_TYPE
			 FROM adi.MbrWlcMbrByPcp m
			 WHERE m.LoadDate = @loadDate
				AND @LoadDate < ISNULL(TERMDATE, '12/31/2099')
	   ...
	   LEFT JOIN (SELECT DISTINCT pr.tin, pr.ClientProviderID
		  ...
		  */
    WHERE @LoadDate BETWEEN EffectiveMembers.EffDate AND EffectiveMembers.TermDate
