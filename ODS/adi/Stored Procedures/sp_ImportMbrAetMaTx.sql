...
-- =============================================
CREATE PROCEDURE [adi].[sp_ImportMbrAetMaTx](
...
--	IF @@ROWCOUNT = 0
 ...
 INSERT INTO [adi].[MbrAetMaTx]
  ...
  CONVERT(date, RIGHT( Substring(@SrcFileName,1, (PATINDEX('%.txt%', @SrcFileName)-1)), 8)),
  --  @DataDate ,
  -- @CreatedDate date,
    GETDATE(),
   @CreatedBy ,
   ...
   --AceMetaData.adi.udf_GetCleanDate (AceMetaData.adi.udf_GetCleanString(@Member_Date_of_Birth)),
   	CASE WHEN (@Member_Date_of_Birth = '')
	THEN NULL
	ELSE CONVERT(date, AceMetaData.adi.udf_GetCleanString(@Member_Date_of_Birth)) 
    END,
   ...
   AceMetaData.adi.udf_GetCleanString(@Funding_Arrangement) ,
	...
	CASE WHEN (@From_attribution_date_of_first_visit = '')
	THEN NULL
	ELSE CONVERT(date, AceMetaData.adi.udf_GetCleanString(@From_attribution_date_of_first_visit))
	END,

	--AceMetaData.adi.udf_GetCleanDate(AceMetaData.adi.udf_GetCleanString(@From_attribution_date_of_first_visit)),
	CASE WHEN (@From_attribution_date_of_last_visit = '')
	THEN NULL
	ELSE CONVERT(date, AceMetaData.adi.udf_GetCleanString(@From_attribution_date_of_last_visit))
	END ,
	...
	CASE WHEN (@Attribution_Original_Effective_Date = '')
	THEN NULL 
	ELSE CONVERT(date, AceMetaData.adi.udf_GetCleanString(@Attribution_Original_Effective_Date))
	END,
	--AceMetaData.adi.udf_GetCleanDate(AceMetaData.adi.udf_GetCleanString(@Attribution_Original_Effective_Date)),
	CASE WHEN (@Attribution_Cancel_Date = '')
	THEN NULL
	ELSE CONVERT(date, AceMetaData.adi.udf_GetCleanString(@Attribution_Cancel_Date))
	END,
