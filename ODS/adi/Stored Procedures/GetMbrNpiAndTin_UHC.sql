...
--SELECT max(src.DataDate)
--FROM		[ACDW_CLMS_UHC].[adi].[UHCMemberbyProvider] src 
--								 
--SELECT max(src.A_LAST_UPDATE_DATE)
--FROM dbo.Uhc_MembersByPcp src
--
...
CREATE PROCEDURE [adi].[GetMbrNpiAndTin_UHC] (@DataDate DATE, @RowStatus INT,@ClientKey INT) -- [adi].[GetMbrNpiAndTin_UHC]'2021-05-17',0,1

AS
					 --DECLARE @DatadDate DATE = '2021-08-23' 
					...
					SELECT	pr.NPI
							...
							,src.SUBSCRIBER_ID 
					FROM		(/* Changed source to dbo/adw store for uhc members, since the scope is adw GK
								SELECT		DISTINCT src.SUBSCRIBER_ID,src.PROVIDER_NPI ,src.IRS_TAX_ID
								 FROM		[ACDW_CLMS_UHC].[adi].[UHCMemberbyProvider] src -- 																 
								 WHERE		DataDate =  @DataDate 
								 */
								SELECT		DISTINCT src.UHC_SUBSCRIBER_ID SUBSCRIBER_ID,src.PCP_NPI PROVIDER_NPI ,src.PCP_PRACTICE_TIN IRS_TAX_ID
								 FROM		dbo.Uhc_MembersByPcp src -- 																 
								 WHERE		A_LAST_UPDATE_DATE =  @DataDate 
								) src
					LEFT JOIN	(SELECT		* 
