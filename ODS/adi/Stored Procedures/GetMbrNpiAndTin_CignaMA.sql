...
CREATE PROCEDURE [adi].[GetMbrNpiAndTin_CignaMA] (@DataDate DATE, @RowStatus INT,@ClientKey INT) -- [adi].[GetMbrNpiAndTin_CignaMA]'2021-06-01',0,12

AS
					-- DECLARE @DataDate DATE = '2021-06-01' DECLARE @RowStatus INT = 0 DECLARE @ClientKey INT = 12
					...
					SELECT	pr.NPI
							...
							,''	AS	ClientTIN 
							,src.memberid 
					FROM		(SELECT		REPLACE(src.memberid,'*','')memberid
											,src.NPID
