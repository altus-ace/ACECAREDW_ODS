CREATE PROCEDURE [adi].[AceValidMbrAet_MbrDetails](
...
AS
	...
	DECLARE  @LoadHistory TABLE (MbrDetailType VARCHAR(100), LoadDate Date,  ClientSubID VARCHAR(50))
	INSERT INTO @LoadHistory
     SELECT 'Adi Load history' AS MbrDetailType,             
            w.LoadDate, 
            @mbrSubID AS ClientSubscriberID
     FROM adi.MbrAetMbrByPcp w	
     WHERE w.AetSubscriberID = @mbrSubID
	GROUP BY w.LoadDate
	...
	select 'add code to validate provider by load date';
/*	SELECT 'Adi PCP Contract Status' AS [Adi.PCPContract Status],             
 ...
 --   
    ...
    SELECT * 
     FROM adi.MbrAetMbrByPcp w
	   CROSS APPLY [adw].[tvf_GetValidProviderTinsByDate](w.LoadDate) AS PL /*ON w.NPI = PL.NPI 
		  AND w.TIN = PL.TIN	   
		  AND w.LOB = pl.LOB*/
     WHERE w.AetSubscriberID = @mbrSubID

	SELECT LoadDate from adi.MbrAetMbrByPcp w WHERE w.AetSubscriber = '231512170036042301';
	*/

	/* XXXXX get member provider details XXXXXX */
	Select 'Add query to get member provider detail + provider validation info';

     SELECT 'adi.MbrAetMbrByPcp' AS [adi.MbrAetMbrByPcp], 
            w.NPI,--pl.NPI ProvNpi, 
            w.TIN, --pl.TIN ProvTin, 
            w.LoadDate, 
            w.mbrAetMbrByPcpKey, 
            w.AetSubscriberID,         		  
		  w.GroupIndicator,               
            ...
            --w.Prov_id, 
            --w.Prov_Name, 
            w.Product as [Product-LOB], 
		  w.SubgroupNumber AS BENE_Plan, 
		  w.SubgroupName,       
            w.SrcFileName, 
            w.InQuarantine, 
            w.CreateDate      
