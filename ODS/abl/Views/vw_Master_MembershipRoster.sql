...
CREATE VIEW [abl].[vw_Master_MembershipRoster]
...
AS				
    ...
    SELECT  
					...
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(AM.MEMBER_LAST_NAME, ''))))			 AS LAST_NAME		 
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(AM.MEMBER_FIRST_NAME,''))))		     AS FIRST_NAME		 
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(AM.MEMBER_MI, ''))))				 AS MIDDLE_NAME	 
					...
					, ''													 AS SSN9
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(am.MEMBER_HOME_ADDRESS,''))))		 AS MEMBER_HOME_ADDRESS
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(am.MEMBER_HOME_ADDRESS2,''))))	     AS MEMBER_HOME_ADDRESS2
					, [adi].[udf_ConvertToCamelCase](RTRIM(LTRIM(ISNULL(am.MEMBER_HOME_CITY,''))))			 AS MEMBER_HOME_CITY
