CREATE VIEW [abl].[vw_GHH_MemberRoster]
...
AS/* Purpose: Gets all Active members with MPI/Ace ID for Export to 3rd parties 
     ...
     History: GK created 11/14/2019
			 ...
			 1/18/2021: GK Made Active MEmbers return Only one row for a ACE ID 
    ...
    SELECT DISTINCT 
		...
		, am.MEMBER_POD_DESC    
     FROM (SELECT AM.Ace_ID, am.MEMBER_LAST_NAME, am.MEMBER_FIRST_NAME, am. MEMBER_MI, am.GENDER, am.DATE_OF_BIRTH
			 , am.MEMBER_HOME_ADDRESS, am.MEMBER_HOME_ADDRESS2, am.MEMBER_HOME_CITY, am.MEMBER_HOME_STATE, am.MEMBER_HOME_ZIP_C
			 , am.MEMBER_HOME_PHONE, am.MEMBER_POD_DESC 
			 , ROW_NUMBER() OVER(PARTITION BY am.Ace_ID ORDER BY am.Ace_ID) arn
