Create PROCEDURE [ast].[PdwCop_Load_Uhc_OldPcor]
AS 
    /* version History: 
   ...
   /* Validation Code: check for missing mapping */ 
    ...
    EXEC @cntMissingMapping = adi.AceValidCopUhcCareOpsPcor 
    ...
    DECLARE @InputSourceName VARCHAR(200) 
	   SELECT @InputSourceName = DB_NAME() + '.adi.vw_pdwSrc_CopUhcPcor'	    
    ...
    SELECT 
	   ...
	   ,'adi.vw_pdwSrc_CopUhcPcor'
	   ,Pcor.adiKey  
	   ,CONVERT(date, GETDATE()) aS LoadDate	   
    FROM adi.[tvf_pdwSrc_CopUhcPcor]((SELECT MAX(stg.DataDate) MaxDataDate FROM adi.vw_pdwSrc_CopUhcPcor stg )) AS Pcor
        JOIN lst.List_Client Client ON 'UHC' = Client.ClientShortName	
	   ;
    ...
    SELECT 
	   ...
	   ,Pcor.adiKey  
	   ,CONVERT(date, GETDATE()) aS LoadDate
    FROM adi.tvf_pdwSrc_CopUhcPcor((SELECT MAX(stg.DataDate) MaxDataDate FROM adi.vw_pdwSrc_CopUhcPcor stg )) Pcor
        JOIN lst.List_Client C ON 'UHC' = c.ClientShortName
	   ;
