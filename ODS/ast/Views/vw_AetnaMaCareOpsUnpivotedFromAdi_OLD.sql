...
CREATE VIEW [ast].[vw_AetnaMaCareOpsUnpivotedFromAdi_OLD]
as
   ...
   UNION
    ...
    SELECT DISTINCT
          ...
          WHEN (TRY_convert(numeric(10,2), co6.DiabetesMedicationPDCYTD)BETWEEN 0.70 AND 0.95) then 'COP'
          else 'UNK' end as qmComp
   ...
   UNION
    ...
    SELECT DISTINCT
          ...
          WHEN (TRY_convert(numeric(10,2), co9.StatinMedicationPDCYTD)BETWEEN 0.70 AND 0.95) then 'COP'
          else 'UNK' end as qmComp
	...
	LEFT JOIN lst.listacemapping m ON REplace(adiData.Measure,' ', '') = replace(m.Source, ' ', '')
	   and m.MappingTypeKey= 14
    LEFT JOIN lst.LIST_QM_Mapping	ListQm ON m.Destination = ListQm.QM
