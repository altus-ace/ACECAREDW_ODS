CREATE FUNCTION [dbo].[tvf_Activemembers_OldRoster](@EffectiveDate DATE) 
RETURNS TABLE
...
--		Left Join [adw].[vw_ActiveMembers_ClientRiskScore] AS  rs
--		  ON	 m.ClientMemberKey = rs.ClientMemberKey
--			 and rs.CLientkey in (2,3,9,11,12)
	where m.ClientKey <> 1
	)s
		 WHERE  RwCnt = 1
 ...
 --declare @EffectiveDate date = '12/15/2020'
	...
	FROM		(
					SELECT		DISTINCT 
								...
								pr.GroupName AS [PCP_PRACTICE_NAME], -- changed to fix permian Tin Issue, after fix fact data, remove this
								'' AS [AUTO_ASSIGN],
								'' AS [MEMBER_STATUS],
								'' AS [MEMBER_TERM_DATE],
					...
					LEFT JOIN adw.fctProviderRoster PR  -- Changed to Left Join to not remove TINS 
					ON c.PcpPracticeTIN = pr.TIN
				    and c.NPI = pr.NPI
				    and c.LoadDate between pr.RowEffectiveDate and pr.RowExpirationDate
					WHERE	Active = 1 
		 ...
		 /*
		 --- Brit - 2021-02-03: Old Code Replaced with above
   SELECT		 
					0 AS [mbrMemberKey], 
					0 AS mbrDemographicKey, 
					0 AS mbrPlanKey,
		 ...
		 FROM		ACDW_CLMS_SHCN_MSSP.adw.FctMembership c
			 -- Added join to fix permian Tin Issue, after fix fact data, remove this
			 LEFT JOIN ACDW_CLMS_SHCN_MSSP.adw.fctProviderRoster PR  -- Changed to Left Join to not remove TINS 
				ON c.PcpPracticeTIN = pr.TIN
				    and c.NPI = pr.NPI
				    and c.LoadDate between pr.RowEffectiveDate and pr.RowExpirationDate
		 --WHERE	@EffectiveDate  BETWEEN c.RwEffectiveDate and c.RwExpirationDate if the effective date is greater than the SCHN exp date, use max exp date
		 WHERE (SELECT CASE WHEN (@effectiveDate > 
				( SELECT Max(c.RwExpirationDate)  FROM ACDW_CLMS_SHCN_MSSP.adw.FctMembership c)) 
				    THEN (SELECT Max(c.RwExpirationDate)  FROM ACDW_CLMS_SHCN_MSSP.adw.FctMembership c)
				    ELSE @EffectiveDate END as aDate)	BETWEEN c.RwEffectiveDate and c.RwExpirationDate
			 AND Active = 1 		 			
