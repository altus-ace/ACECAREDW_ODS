...
CREATE FUNCTION [dbo].[tmp_ACE_tvf_current_UHC_CareOpps]
 
(	@anchor_mth int,@anchor_year int, @pcp varchar(15)
)
RETURNS TABLE 
As
RETURN
...
case when c.uhc_subscriber_id is null then 'NO' else 'YES' end as NewMbr
FROM [ACECAREDW].[dbo].[vw_current_UHC_CareOpps] a
join [ACECAREDW].[dbo].[vw_UHC_ActiveMembers] b on a.memberID = b.Member_id
left join (SELECT distinct e2.UHC_SUBSCRIBER_ID
...
where update_month = @anchor_mth and Update_year= @anchor_year and a.Provider_NPI = @pcp
)
