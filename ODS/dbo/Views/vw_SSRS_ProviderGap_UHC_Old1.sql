...
CREATE VIEW [dbo].[vw_SSRS_ProviderGap_UHC_Old1]
AS
     ...
     (
         SELECT DISTINCT 
                ...
                CASE
                    ...
                    ELSE tzip.[zone]
                END AS [Zone]
         FROM /*(SELECT qm.QM_ResultByMbr_HistoryKey, QmMsrId, QMDate, QM.ClientMemberKey, qm.ClientKey
					, CASE WHEN (QM.QmMsrId = 'UHC_CDC_G_9' AND qm.QmCntCat = 'NUM') THEN 'COP'
						WHEN (QM.QmMsrId = 'UHC_CDC_G_9' AND qm.QmCntCat = 'COP') THEN 'NUM'
						ELSE qm.QmCntCat END qmCntCat--, QmCntCat AS SrcQmCntCat
				FROM [ACECAREDW].[adw].[QM_ResultByMember_History] QM
				)
				*/
			  [adw].[QM_ResultByMember_History]  QM
