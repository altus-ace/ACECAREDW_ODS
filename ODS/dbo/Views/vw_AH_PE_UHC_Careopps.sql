...
CREATE VIEW [dbo].[vw_AH_PE_UHC_Careopps]
AS
(
/*
    GET ACTIVE_Members
    ...
    */
    ...
    FROM (SELECT AM.CLIENT_SUBSCRIBER_ID, am.SUBGRP_ID, am.DATE_OF_BIRTH, am.CurMonthsOld
    		  ...
    		  WHERE NOT AM.clientkey in (1, 3, 12)  -- as care op exports are moved to the ahsExpCareOpToProgram add here 
    	   UNION
    		  SELECT am.MEMBER_ID AS UHC_SUBSCRIBER_ID, am.SUBGRP_ID, am.DATE_OF_BIRTH, am.CurMonthsOld
    			 , DATEADD(Month, 15, am.DATE_OF_BIRTH) as WCV_ProgEndDate
    			 , CASE WHEN (am.CurMonthsOld<=15) THEN 1 ELSE 0 END AS WCV_IsLess16Months    
    ...
    SELECT 
       ...
       cop.DESTINATION_PROGRAM_NAME, 
	  cop.ClientMemberKey AS MEMBER_ID,
       cop.programStartDate, 
       cop.ProgramCreateDate, 
       cop.ProgramEndDate, 
       cop.ProgramStatusCode, 
       cop.ReasonDescription, 
       cop.ReferalType,
	  cop.ClientKey
    FROM [adw].[AhsExpCareOpsToPrograms] cop
	   JOIN lst.list_client client ON cop.ClientKey = client.ClientKey	   
    WHERE cop.Exported = 0
	   and cop.CareOpToProgActive = 'Y'
	   and COP.ClientKey in ( 3, 12)	
    UNION 
    ...
    SELECT co.CS_Export_LobName AS CLIENT_ID
	  ...
	  ,co.ENROLL_DATE		  AS ENROLL_DATE
       ...
       ,co.REFERAL_TYPE		  AS REFERAL_TYPE
       ,co.ClientKey		  AS ClientKey
    FROM ACDW_CLMS_SHCN_BCBS.adw.vw_Exp_QmPrograms AS co
    UNION 
    SELECT co.CS_Export_LobName AS CLIENT_ID
       ,co.PROGRAM_NAME		  AS PROGRAM_NAME
       ,co.MEMBER_ID		  AS MEMBER_ID
	  ,co.ENROLL_DATE		  AS ENROLL_DATE
       ...
       ,co.REFERAL_TYPE		  AS REFERAL_TYPE
       ,co.ClientKey		  AS ClientKey
    FROM ACDW_CLMS_AMGTX_MA.adw.vw_Exp_QmPrograms AS co
    UNION 
    SELECT co.CS_Export_LobName AS CLIENT_ID
       ,co.PROGRAM_NAME		  AS PROGRAM_NAME
       ,co.MEMBER_ID		  AS MEMBER_ID
	  ,co.ENROLL_DATE		  AS ENROLL_DATE
       ...
       ,co.REFERAL_TYPE		  AS REFERAL_TYPE
       ,co.ClientKey		  AS ClientKey
    FROM ACDW_CLMS_AMGTX_MCD.adw.vw_Exp_QmPrograms AS co
)  
