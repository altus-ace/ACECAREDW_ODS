...
CREATE VIEW [dbo].[vw_RP_WellCareRosterProviderLocations]
AS
SELECT DISTINCT
...
FROM TMPSALESFORCE_CONTACT
     ...
     LEFT JOIN tmpSalesforce_Location__c ON tmpSalesforce_Location__c.[Provider_Name__c] = TMPSALESFORCE_CONTACT.ID
     	...
     	inner JOIN [tmpSalesforce_Contract_Information__c] tc on tc.Account_Name__c=tmpSalesforce_Account.Id and tc.Health_Plans__c='Wellcare'
WHERE 
