...
Created this based on the logic simialr to Devoted - [dbo].[vw_Devoted_ProviderRoster]
...
CREATE VIEW [dbo].[vw_SHCN_MSSP_ProviderRoster]
AS
SELECT DISTINCT
	  ...
	  tci.Line_of_business__C as LOB,
	  tci.effective_date__C,
	  tci.term_date__C,
	  tmpSalesforce_Account.Type__c,
	  Chapter
...
FROM TMPSALESFORCE_CONTACT
     ...
     LEFT JOIN dbo.tmpSalesforce_Location__c on tmpSalesforce_Location__c.Practice_tax_id__C = tmpSalesforce_Account.tax_id_number__C
...
WHERE tmpSalesforce_Account.Type__c like '%SHCN%'
...
AND tmpsalesforce_Account.in_Network__c = '1'
and TCI.Health_Plans__c = 'SHCN_MSSP'
