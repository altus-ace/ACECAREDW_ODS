...
CREATE VIEW [dbo].[zz_vw_AH_IPNotification_New]
AS 
     /* VERSION HISTORY
    11/17/2020: GK Ticket 2634 Shcn Ip: change filter treat SHCN like all other clients, so SHCN sourced NTF are exported, version saved as ...old_11172020
    */
    SELECT    
...
WHERE	
   ...
   ( (ntf.ClientKey <> 16 AND (ntf.AceFollowUpDueDate >= GETDATE() AND (NTF.CaseType <> 'Inhospital' )))    -- followup = dischare + interval from client: Datadate
    OR (ntf.ClientKey = 16 AND (ntf.AceFollowUpDueDate >= GETDATE() AND ntf.DschrgInferredDate <> '01/01/1900')))
