CREATE VIEW [dbo].[z_vw_RP_LOBPracticeActiveMembership_HQ_DEV2]
AS
    ...
    gk: 01/21: add JOIN on
     ...
     FROM(SELECT DISTINCT S.TIN, s.CLIENT
	   , s.ProviderGroupName PRACTICE_NAME , s.AccountType, s.NetworkContact	   
	   , s.PrimaryState /*, s.PrimaryPOD*/, S.MEMBER_ID
         ...
         RIGHT JOIN (SELECT DISTINCT a.Member_id, client.clientshortname AS Client, CONVERT(INT, a.pcp_practice_tin) AS TIN, a.NPI, ISNULL(prNpi.GroupName, '') As PracticeName
--				    , a.PCP_POD_C 
				    ...
				    , CASE WHEN (prNpi.PrimaryState is null) THEN PrTin.PrimaryState 
					   ELSE PrNpi.PrimaryState END as PrimaryState 
