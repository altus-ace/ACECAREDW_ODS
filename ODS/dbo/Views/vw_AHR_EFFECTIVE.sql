CREATE VIEW dbo.vw_AHR_EFFECTIVE 
AS 
SELECT 
	   D.[Context]
      ,D.[TIN_Num]
      ...
      ,D.[Gender]
      ,D.[Provider_NPI]
      ,D.Latest_Shown_on_CareOPP
	  ,D.CLIENT_PATIENT_ID
	  ,D.Latest_AppDate
	  ,D.GAP_CLOSE_TAG
FROM (
SELECT * FROM (SELECT DISTINCT TIN_Num,TIN_Name, A_LAST_UPDATE_DATE,
ROW_NUMBER() OVER (PARTITION BY TIN_Num ORDER BY A_LAST_UPDATE_DATE DESC) AS RANK 
