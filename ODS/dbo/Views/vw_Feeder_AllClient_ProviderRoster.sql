...
CREATE VIEW [dbo].[vw_Feeder_AllClient_ProviderRoster]
AS
...
SELECT DISTINCT
	   ...
	   TMPSALESFORCE_ACCOUNT.Type__c as SPECIALTY_TYPE,
...
WHERE tmpSalesforce_Contact.Status__c IN ('ACTIVE')
      AND tmpsalesforce_Contact.Type__c IN('PCP', 'UHC PCP', 'Specialist','Multi-specialty')
     AND LASTNAME NOT IN('TEST', 'TESTLAST', 'ACETEST')
	-- AND TCI.Health_Plans__c = 'Aetna' --and tci.Line_of_business__C in ('Medicare','Medicare Advantage')
AND tmpsalesforce_Account.in_Network__c = '1';
