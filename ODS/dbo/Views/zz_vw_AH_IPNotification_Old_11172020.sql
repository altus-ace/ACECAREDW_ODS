...
CREATE VIEW [dbo].[zz_vw_AH_IPNotification_Old_11172020]
AS 
SELECT 
...
FROM		adw.NtfNotification ntf   
    JOIN	(SELECT 
			 ...
			 , AC_AM.MEMBER_MAIL_PHONE OtherPhone
			 FROM dbo.vw_ActiveMembers AC_AM
		  Union ALL
		  SELECT 	
			   SM_AM.ClientMemberKey AS MEMBER_ID
			 , ISNULL(SM_AM.providerFirstName,'') + ' ' + ISNULL(SM_AM.ProviderLastName,'') AS PCP_NAME
			 , SM_AM.ProviderPracticeName AS PCP_PRACTICE_NAME
			 , SM_AM.FirstName AS MEMBER_FIRST_NAME
			 , SM_AM.LastName	  AS MEMBER_LAST_NAME
			 , SM_AM.DOB		  AS DATE_OF_BIRTH
			 , SM_AM.ProviderChapter AS AhsPlanName
			 , SM_AM.MemberHomePhone HomePhone
			 , SM_AM.MemberCellPhone OtherPhone
			 FROM ACDW_CLMS_SHCN_MSSP.[adw].[2020_tvf_Get_ActiveMembersFull]('05/15/2020') AS SM_AM
		  ) AS am
