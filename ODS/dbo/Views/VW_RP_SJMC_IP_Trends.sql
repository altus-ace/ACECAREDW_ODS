 ...
 /*
 BRIT:2021-01-07: Added derived table of unioned and finaloutput set to create a makeshift for delayed UHC membership file for Dec2020
 */


CREATE VIEW [dbo].[VW_RP_SJMC_IP_Trends]   
AS
--     -- from claims
	SELECT			*
	FROM			(
			SELECT			*, ROW_NUMBER() OVER(PARTITION BY Member_ID, VEND_FULL_NAME, VISITDATE ORDER BY VISITDATE DESC, pcp_practice_name DESC) RwCnt
			FROM			(
					SELECT DISTINCT 
					       'UHC' AS Client, 
...
--					         ee.MULTI_CCS_LVL1_LABEL,
					...
					WHERE CATEGORY_OF_SVC = 'INPATIENT'
					    ...
					    --AND YEAR(PRIMARY_SVC_DATE) = 2020
					    --AND MONTH(primary_svc_date) = 12
					...
					WHERE
					 DischargeDate >= '1/1/2018'
					 --YEAR(DischargeDate) = 2020
