...
CREATE VIEW [dbo].[vw_RP_8x8CRM_Callcenter]
AS
    ...
    VERSION HISTORY:
	   ...
	   06/03/2021 GK Altered column source for LineOfBusiness for certain (fctMbrshp) clients to use the SUBGRP_NAME column instead of the Plan_Desc column
				    */
    ...
    FROM (	   
	   SELECT distinct 
	   	 ...
	   	 CASE WHEN (ISNULL(RTRIM(LTRIM(PrimaryPhone.Phone_number)), '') = '') then ''
	   	  ...
	   	  CASE WHEN ((SUBSTRING(ISNULL(RTRIM(LTRIM(TinAdditionalPhones.tinphone )), ''), 1,1) <> 1) AND (NOT TinAdditionalPhones.TinPhone IS NULL))THEN concat(1,TinAdditionalPhones.tinphone)
	          ...
	          END AS MCOProduct,
			CASE WHEN ActiveMembers.ClientKey IN (2,3, 9, 11, 12) THEN ActiveMembers.Plan_Desc 
				WHEN ActiveMembers.ClientKey IN (16, 20, 21, 22) THEN ActiveMembers.SUBGRP_NAME
				WHEN ActiveMembers.clientKey IN (1) THEN ActiveMembers.AhsPlanName
				ELSE 'UNKNOWN LOB' END  AS LineOfBusiness,
	   ...
	   FROM  (SELECT am.member_id, am.PCP_PRACTICE_TIN, adw.AceCleanPhoneNumber(am.PCP_PHONE) PCP_PHONE, am.RESP_LAST_NAME, am.RESP_FIRST_NAME ,
	   			...
	   			am.MEMBER_FIRST_NAME, am.MEMBER_MI, am.member_last_name ,am.SUBGRP_NAME ,
	   			am.clientKey, am.LANG_CODE, am.AGE, am.MEMBER_CUR_EFF_DATE, am.MEMBER_CUR_TERM_DATE, am.PLAN_DESC, am.CLIENT, am.AhsPlanName, am.AhsPlanCode
	   			FROM acecaredw.dbo.vw_ActiveMembers am
				) ActiveMembers -- 31534	-- 31534	
