...
CREATE VIEW [dbo].[vw_ACE_Validation_Chk_WLC_Mbrcsplanmbrchanges]
AS
...
SELECT Sub_id,
       ...
       old_effDate,
       old_plan,
	  case when cur_plan <> mp.MbrCsSubPlan  then 1 else 0 end as csplanchangeerror,
       mp.MbrCsSubPlan,
       mp.EffectiveDate,
       mp.ExpirationDate
...
inner JOIN ACECAREDW.adw.mbrCsPlanHistory mp ON mp.mbrMemberKey = m.mbrMemberKey and mp.ExpirationDate='2099-12-31'
WHERE a.change = 1) as a where a.csplanchangeerror=1
