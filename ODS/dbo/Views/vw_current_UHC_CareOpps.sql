...
select a.MemberID,a.update_month ,
	   update_Year
        ...
        ,max([DOB]) as DOB
	    ,max(pcp_practice_tin) as TIN_Num
	    ,MAX(PCP_PRACTICE_NAME) AS TIN_NAME
	    ,max([ProviderID])as ProviderID
	    ,max(PCP_NPI) as Provider_NPI
		,max(PCP_LAST_NAME) as Prov_Last_Name
        ,max(PCP_FIRST_NAME) as Prov_First_Name
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vw_current_UHC_CareOpps';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vw_current_UHC_CareOpps';
