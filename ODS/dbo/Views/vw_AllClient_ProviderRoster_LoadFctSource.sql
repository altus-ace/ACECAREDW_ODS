...
CREATE VIEW [dbo].[vw_AllClient_ProviderRoster_LoadFctSource]
AS    
    /* Objective: Extract from the SalesForce Data Dump tables, the currently loaded set of providers 
	   ...
	   01/15/2020 gk 1. Added BCBS client processing 2. added PrimaryCounty to allow bcbs market/chapter mapping to occur.
	   02/02/2020:gk: converted Lob for Aetna and Cigna to use like to improve data in clientkey sets (was excluding Medicare/Commercial values as that isn't commercial explicitly)
	   */
    SELECT *, 	   -- Ignore  clients --4 UHC-MI, --5 UHC-CP-PCPi, --6 CCACO --7 OKC, --8	ECAP, --13 mpulse, --14 Ace
	 CASE WHEN (PR.HealthPlan = 'AETNA'	   and 	PR.LOB like '%Commercial%')				THEN  9	  --9	Aetna Comercial	AetCom
		  ...
		  WHEN (PR.HealthPlan = 'CIGNA'	   and 	PR.LOB = 'Commercial')					THEN  17	  -- 17	CIGNA Commercial	CignaCom
		  WHEN (PR.HealthPlan = 'MOLINA'	   and 	PR.LOB = 'Market Place')					THEN  19	  --19	Molina	Mol
		  WHEN (PR.HealthPlan = 'MOLINA'	   and 	PR.LOB = 'Medicaid')					THEN  19	  --19	Molina	Mol
		  ...
		  WHEN (PR.HealthPlan = 'OSCAR'	   and 	PR.LOB = 'Medicare Advantage')			THEN  15	  --15	Oscar Health	Oscar
...
--	   	  CONVERT(DATE, AceMetaData.[adi].[udf_GetCleanDate](ISNULL(SF_ProviderClient.Term_Date__c, '12/31/2099'))) AS ExpirationDate,
		  ...
		  , AceMetaData.[adi].[udf_GetCleanString](PrimaryPracticeLocation.County__c) AS PrimaryCounty
