...
CREATE VIEW [adw].[vw_Dashboard_QM_UHC]
AS

     /* OBJECTIVE: GET QUALITY MEASURE FOR A MEMBER BY MONTH FOR A CLIENT */

     SELECT S.ClientMemberKey, 
            S.CareOpToPlan_Active, 
            S.QmMsr_Tb_Desc, 
            S.AHR_QM_DESC, 
            S.QmMsrId, 
            S.QMDate, 
            S.LOB, 
            S.MeasureDesc, 
            SUM(S.MbrDEN) AS MbrDEN, 
            SUM(S.MbrNUM) AS MbrNUM, 
            SUM(S.MbrCOP) AS MbrCOP, 
            S.AGE, 
            S.PCP_PRACTICE_TIN, 
            S.PCP_PRACTICE_NAME, 
            s.ProviderAccountType, 
            S.PCP_NPI, 
            S.PCP_PHONE, 
            S.careOpToPlan_MeasureID, 
            S.[Contract?], 
            S.[ACE MBR?], 
            S.ExpirationYear, 
			CASE 
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_CCS'
			Then '0.5742'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_CBP'
			THEN '0.4769'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_CDC_L_8'
			THEN '0.4769'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_FUH'
			THEN '0.5915'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_IMA'
			THEN '0.2556'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_PPC_POST'
			THEN '0.7372'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_W0014'
			THEN '0.6000'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_W1530'
			THEN '0.6000'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_WCC_CN'
			THEN '0.5231'
			when S.CAREOPTOPLAN_MEASUREID = 'UHC_WCV'
			THEN '0.6491'
            ELSE S.Target
			END as Target, 
            S.Client, 
     ...
     (
         SELECT [CLIENTMEMBERKEY], 
                ...
                CASE -- CLEANED UP TO MATCH THE UHC 2020 MEASURES 
					...
					THEN 'Comprehensive Diabetes Care - HbA1c control (<8.0%)'
					WHEN [QMMSRID] = 'UHC_FUH'
					THEN 'Follow-Up after Hospitalization for Mental Illness'
					WHEN [QMMSRID] = 'UHC_PPC_POST'
					THEN 'Prenatal & Postpartum Care'
					WHEN [QMMSRID] = 'UHC_W0014'
					THEN 'Well Child Visits in the 0 to 14 Months of Life'
					WHEN [QMMSRID] = 'UHC_W1530'
					THEN 'Well Child Visits in the 15 to 30 Months of Life'
					WHEN [QMMSRID] = 'UHC_WCC_CN'
					THEN 'Weight Assessment & Counseling for Nutrition & Physical Activity for Children/Adolescent'
					WHEN [QMMSRID] = 'UHC_WCV'
					THEN 'Adolescent WellCare Visits'
                    ELSE 'O'
         ...
         (
             SELECT DISTINCT 
                    ...
                    CASE
                        WHEN(MP.PCP_PRACTICE_TIN = '134334288')
                        THEN PR_SHCN.GROUPNAME
                        ELSE PR.GROUPNAME
                    END AS PCP_PRACTICE_NAME,
                    CASE
                        WHEN(PR.ACCOUNTTYPE LIKE 'SHCN_%')
                        THEN PR.ACCOUNTTYPE
                        WHEN(PR.AccountType IS NULL)
                        THEN NULL
                        ELSE 'ACE'
                    END AS ProviderAccountType, 
                    MP.[PCP_NPI], 
                    MP.[PCP_PHONE], 
                    ...
                    CASE
                        ...
                        THEN 'INACTIVE'
                        ELSE 'ACTIVE'
                    END AS TinInContractWithAce
             FROM [ADW].[QM_RESULTBYMEMBER_HISTORY] QM
                  LEFT JOIN DBO.VW_ACTIVEMEMBERS AM ON AM.MEMBER_ID = QM.CLIENTMEMBERKEY -- NEED TO ADD THIS TO REMOVE MEMBERS WHO ARE WRONGLY PULLED INTO PCOR WITH PLANS IN MMP NAMED AS STARPLUS
                  ...
                  LEFT JOIN
             (SELECT pr.attribTin TIN, pr.attribTinName GROUPNAME, pr.AccountType
				FROM adw.tvf_allClient_ProviderRoster(1, getdate(), 0) pr
				GROUP BY pr.attribTin, pr.attribTinName, pr.AccountType                 
				 /* old roster 
				 SELECT PR.TIN, 
                        PR.GROUPNAME, 
                        PR.ACCOUNTTYPE
                 FROM DBO.yy_VW_ALLCLIENT_PROVIDERROSTER PR
                 WHERE PR.CALCCLIENTKEY = 1 --AND PR.TIN <> '134334288'                                                                                                                       
                 GROUP BY PR.TIN, 
                          PR.GROUPNAME, 
                          PR.ACCOUNTTYPE
						  */
             ) PR ON MP.PCP_PRACTICE_TIN = PR.TIN
                  LEFT JOIN
             (SELECT pr.attribTin TIN, pr.attribTinName GROUPNAME, PR.NPI, pr.AccountType
				FROM adw.tvf_allClient_ProviderRoster(1, getdate(), 0) pr
				WHERE pr.attribTin = '134334288'
				GROUP BY pr.attribTin, pr.attribTinName,PR.NPI, pr.AccountType          
				/* old roster 
                 SELECT PR.TIN, 
                        PR.GROUPNAME, 
                        PR.NPI, 
                 ...
                 WHERE PR.CALCCLIENTKEY = 1
                       AND PR.TIN = '134334288'
                 GROUP BY PR.TIN, 
                          PR.GROUPNAME, 
                          PR.NPI, 
                          PR.ACCOUNTTYPE
						  */
             ) PR_SHCN ON MP.PCP_PRACTICE_TIN = PR_SHCN.TIN
	...
	-- WHERE Year(s.qmdate)>= 2021
     GROUP BY S.CLIENTMEMBERKEY, 
              S.CAREOPTOPLAN_ACTIVE, 
              S.QMMSR_TB_DESC, 
              S.AHR_QM_DESC, 
              S.QMMSRID, 
