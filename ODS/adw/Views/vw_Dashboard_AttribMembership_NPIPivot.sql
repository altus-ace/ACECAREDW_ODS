...
CREATE VIEW [adw].[vw_Dashboard_AttribMembership_NPIPivot]
AS
...
--SELECT DISTINCT NPI,CONCAT(LastName,',',FirstName) as FullName, CASE WHEN AccountType IN ('SHCN_SMG','SHCN_AFF') THEN AccountType ELSE 'ACE' END as AcctType, Chapter, TIN --, GroupName
...
SELECT EffectiveAsOfDate, AttribNPI, FullName, AcctType, Chapter, Practice
	,REPLACE(REPLACE (UHCMedicaid,99999,'-')	,99998,'--')				as UHCMedicaid	
	,REPLACE(REPLACE (MSSP,99999,'-') 			,99998,'--')				as MSSP		 
	,REPLACE(REPLACE (AetnaMA,99999,'-') 		,99998,'--')				as AetnaMA	 
	,REPLACE(REPLACE (CignaMA,99999,'-') 		,99998,'--')				as CignaMA	 
	,REPLACE(REPLACE (DevotedMA,99999,'-') 	,99998,'--')				as DevotedMA	
	,REPLACE(REPLACE (WellcareMA,99999,'-') 	,99998,'--')				as WellcareMA 
	,REPLACE(REPLACE (AetnaComm,99999,'-') 	,99998,'--')				as AetnaComm	
	,REPLACE(REPLACE (BCBSComm,99999,'-') 		,99998,'--')				as BCBSComm	 
	,REPLACE(REPLACE (AmerigroupMA,99999,'-') 		,99998,'--')				as AmerigroupMA
	,REPLACE(REPLACE (AmerigroupMedicaid ,99999,'-') 		,99998,'--')		as AmerigroupMedicaid 
	,REPLACE(REPLACE (Other,99999,'-') 			,99998,'--')				as Other		 
	,REPLACE(REPLACE (Total,99999,'-') 			,99998,'--')				as Total
FROM [adw].[vw_Dashboard_AttribMembership_NPIPivot] p
ORDER BY FullName
...
SELECT	 MbrNPI
			...
			,Sum(Total		)		as Total
FROM #tmpCTE m
JOIN adw.tvf_AllClient_ProviderRoster (0,'2021-05-03',1) p
ON m.MbrNPI = p.NPI
GROUP BY MbrNPI
