...
BEGIN
	...
	DECLARE @FirstMemberMonth DATE; 
	DECLARE @MaxMemberMonth date;

	SELECT @FirstMemberMonth  =  min(m.LoadDate)
	FROM ADW.MbrMember M
	WHERE M.ClientKey = @ClientKey
	;

	SELECT @MaxMemberMonth = s.lastDayOfMonth
	FROM (SELECT DATEADD(day, -1, Dateadd(month, 1, src.MaxMbrMnth)) lastDayOfMonth
		  FROM (SELECT MAX(lh.LoadDate) MaxMbrMnth
