...
CREATE		PROCEDURE [adw].[NtfNotificationCalcDischarges]
...
--A
		...
		WHERE			NtfSource IN ('Shcn_MSSP', 'SHCN_BCBS')
		...
		AND				NtfSource IN ('Shcn_MSSP', 'SHCN_BCBS')
						)a 
...
BEGIN
		...
		FROM				(
								SELECT				CONVERT(DATE,GETDATE())  LoadDate
													,DataDate
													,a.ClientKey
													...
													, b.[srcDiagnosisCode]
													, b.[srcAdmitHospital]
													, CASE			WHEN NtfSource IN ('SHCN_MSSP', 'GHH','SHCN_BCBS' )
																	THEN (	SELECT CONVERT(DATE,DATEADD(DD,(SELECT IpDischargeFollupIntervalInDays  
																			FROM lst.List_Client WHERE ClientKey IN (16)),CONVERT(DATE,GETDATE())))
																		 )
...
BEGIN
		...
		AND			a.ClientKey IN (16 ,20)
		AND			a.NtfSource IN ('SHCN_MSSP','SHCN_BCBS')
