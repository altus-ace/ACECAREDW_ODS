CREATE PROCEDURE [adw].[PdwMbr_06_LoadPcp_ORIGINAL]
    @LoadDate DATE  ,
...
AS
    ...
    SELECT lh.mbrLoadHistoryKey, mbr.mbrMemberKey, p.mbrPcpKey, lh.LoadType		  		   
		  ...
		  , p.EffectiveDate, p.ExpirationDate, pl.NPI AS NewPcpNpi, pl.TIN, p.AutoAssigned
	   ...
	   FROM (SELECT s.ClientSubscriberId, s.AdiKey, s.AdiTableName, s.prvNpi, s.prvTin, s.prvAutoAssign
			 ...
			 FROM (SELECT d.ClientSubscriberId, d.adiKey, d.AdiTableName, d.LoadType      
				,d.prvNPI, d.prvTIN, d.prvAutoAssign, d.prvClientEffective, d.prvClientExpiration
				, d.LoadDate , d.DataDate
				, ROW_NUMBER() OVER (PARTITION BY d.ClientSubscriberId ORDER BY d.LoadDATE DESC) arn				
				FROM ast.MbrStg2_MbrData d)  s
			 WHERE s.arn = 1			
			 ) AS m
		  ...
		  LEFT JOIN adw.MbrWlcProviderLookup pl ON m.prvNPI = pl.NPI
	   WHERE m.LoadDate = @LoadDate
		  AND p.mbrPcpKey is null
		  ;
    ...
    UPDATE adw.MbrPcp
	   SET ExpirationDate = src.NewExpirationDate    
