...
SELECT DISTINCT CONVERT(date,p.DataDate), COUNT(*) FROM ACDW_CLMS_SHCN_BCBS.[adi].[Steward_BCBS_Membership] p 
...
DECLARE @AdiMbrTable			VARCHAR(50) = 'ACDW_CLMS_SHCN_BCBS.adi.Steward_BCBS_Membership'
DECLARE @DataDate				VARCHAR(50) = 'DataDate'
DECLARE @ClientKey			VARCHAR(5)	= '20'
DECLARE @AdiKey				VARCHAR(50) = 'MembershipKey'
DECLARE @ClientMemberKey	VARCHAR(50) = 'PatientID'
DECLARE @AttribNPI			VARCHAR(50) = 'AttributedPrimaryCareProviderNPI'
DECLARE @EffDate				VARCHAR(50) = 'MemberOriginalEffectiveDate'									-- Original Effective Date
DECLARE @CurEffDate			VARCHAR(50) = 'MemberEffectiveDate'
DECLARE @TermDate				VARCHAR(50) = 'MemberTerminateDate'
DECLARE @adiPlan				VARCHAR(50) = 'ProgramIndicator'
...
FROM ' + @AdiMbrTable + ' p 
LEFT JOIN ACDW_CLMS_SHCN_BCBS.[adi].[Steward_BCBS_MemberCrosswalk] c
	ON p.PatientID		= c.PatientID
	AND p.DataDate		= c.DataDate
	AND c.Indicator834	= ' + char(39) + 'Y' + char(39) + '	
LEFT JOIN lst.lstPlanMapping l ON c.' + @adiPlan + ' = l.SourceValue
...
WHERE p.' + @ClientMemberKey + ' IS NOT NULL 
	...
	AND p.MemberAttributedStatus IN (' + char(39) + 'ADD' + char(39) + ',' + char(39) + 'CONTINUE'+ char(39) + ','+ char(39) + 'REINSTATE'+ char(39) + ')
...
END
/***
EXEC adw.z_CreateBCBSMembers '2020-1-01','2021-05-30','2021-06-03'

SELECT * FROM dbo.z_tmp_AttribMembers			WHERE	ClientKey IN (20) AND EffectiveAsOfDate	= '2021-06-03'
SELECT * FROM dbo.z_tmp_AttribMembers_Failed WHERE	ClientKey IN (20)	AND EffectiveAsOfDate	= '2021-06-03'

SELECT * FROM ACECAREDW.adw.tvf_AllClient_ProviderRoster (20,getdate(),1)
***/
