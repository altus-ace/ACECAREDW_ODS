...
--exec [adw].[Pdw_ExpAhs_AhsEligiblity_AetCom]

CREATE PROCEDURE [adw].[Pdw_ExpAhs_AhsEligiblity_AetCom]
    --(@ClientKey INT )
AS 
BEGIN
	/* objective 
		...
		*/
	DECLARE @FirstMemberMonth DATE = '2019-06-14';  /* this is the first month that has valid data, earlier data is not REAL */
	DECLARE @ClientKey INT = 9;
	DECLARE @MaxMemberMonth date;
	SELECT @MaxMemberMonth = MAX(lh.LoadDate)
	...
	--IF @ClientKey = 1, 16, 22,21, 20 THEN BLOW UP
	BEGIN
	...
	FROM ( /* use date dim to generate a member month per month starting with the earliest load for this client to the max for this client*/ 
		  SELECT d.dDate LoadDate
		  FROM ACDW_CLMS_SHCN_MSSP.adw.dimDate d
		  WHERE d.dDay = 15 and d.dDate BETWEEN @FirstMemberMonth and @MaxMemberMonth
		   ) mbr
